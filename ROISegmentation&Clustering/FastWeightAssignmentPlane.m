function [RegConf, vmap, roidfdf] = FastWeightAssignmentPlane(filepath,nClust)
tic
env.nHeight = 2048;
env.nWidth = 2048;
env.nDepth = 1;
...
imagesc(vmap,[0 nClust])
savefig([filepath 'RegConfs.fig']);
% subplot(2,2,2)
...
for iClust = 1:nClust
    ...
    figure(iClust);clf
    imagesc(vmapi, [0 1])
    savefig([filepath 'RegConf' num2str(iClust,'%02d') '.fig']);
%     subplot(2,2,1)
%     imagesc(vmapi(:,:,2),[0 1])
%     subplot(2,2,2)
%     imagesc(vmapi(:,:,9),[0 1])
...
%     imagesc(vmapi(:,:,22),[0 1])
    ...
    savefig([filepath 'FeedbackSimulation' num2str(iClust,'%02d') '.fig']);    
    RegConf.weight = zeros(length(RegConf.Group),1);
end
    toc
end