...
def start_buttonClick():
    ...
    msgText.insert('insert',str(s)+"\n")
    ####add in 20210928
    ser.write(b'OTP P') #External Trigger Output Polarity：poseedge
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    
    ser.write(b'OTW 10US') #External Trigger Output Pulse Width:10us
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    
    ser.write(b'OTS RE/P1') #Reference timing for Programble Trigger Out:End of sensor readout
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    
    ser.write(b'OTS VS/P2') #Reference timing for Programble Trigger Out:Beginning of Vsync
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    
    ser.write(b'OSS P1/1') #Switch External Output Signal:P1/1
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    
    ser.write(b'OSS P2/2') #Switch External Output Signal:P2/2
    ser.write(b'\x0d')
    s=ser.read(20)
    msgText.insert('insert',str(s)+"\n")
    ######add in 20210928
